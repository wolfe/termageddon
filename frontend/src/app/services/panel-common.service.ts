...
export class PanelCommonService implements OnDestroy {
  ...
  private searchSubjects = new Map<
    PanelState,
    Subject<{ searchTerm: string; options: DraftLoadOptions }>
  >();
  ...
  confirmReviewerSelection(reviewerIds: number[], state: PanelState): void {
    ...
    this.unifiedDraftService
      .requestReview(state.draftToRequestReview.id, reviewerIds)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: () => {
      ...
      searchSubject
        ...
        .subscribe({
          next: (response: PaginatedResponse<ReviewDraft>) => {
            ...
            if (
              state.selectedDraft &&
              !state.filteredDrafts.find(d => d.id === state.selectedDraft!.id)
            ) {
              state.selectedDraft =
                state.filteredDrafts.length > 0 ? state.filteredDrafts[0] : null;
            }
          },
          error: error => {
      ...
      .subscribe({
        next: updatedDraft => {
          this.notificationService.success(
            `Draft for "${draft.entry.term.text}" published successfully!`
          );
  ...
  requestReview(
    ...
    this.unifiedDraftService
      .requestReview(draftId, reviewerIds)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: updatedDraft => {
          this.notificationService.success(`Review requests for "${termName}" sent successfully!`);
  ...
  loadDrafts(
    ...
    postProcessFn?: (drafts: ReviewDraft[]) => ReviewDraft[]
  ): void {
    state.loading = true;
    state.error = null;
   ...
   */
  private preserveSelectionAfterLoad(
    state: PanelState,
    currentSelectedDraftId: number | undefined,
    route: ActivatedRoute
  ): void {
    ...
    if (currentSelectedDraftId && state.drafts.length > 0) {
      ...
      if (stillExists) {
        ...
        state.selectedDraft =
          state.drafts.find(draft => draft.id === currentSelectedDraftId) || null;
  ...
  approveDraft(draft: ReviewDraft, state: PanelState, successCallback: () => void): void {
    ...
    this.unifiedDraftService
      ...
      .subscribe({
        next: updatedDraft => {
          this.notificationService.success(`Successfully approved "${draft.entry.term.text}"`);
